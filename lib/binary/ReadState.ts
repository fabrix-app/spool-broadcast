 ...
 * @param {Buffer} buffer
 */
export function ReadState(buffer) {
  /**
   * @member {Buffer}
   * @private
   */
...
ReadState.prototype.readUInt16 = function () {
  const r = this._buffer.readUInt16BE(this._offset)
  this._offset += 2
  return r
}
...
ReadState.prototype.readUInt32 = function () {
  const r = this._buffer.readUInt32BE(this._offset)
  this._offset += 4
  return r
}
...
ReadState.prototype.readDouble = function () {
  const r = this._buffer.readDoubleBE(this._offset)
  this._offset += 8
  return r
}
...
ReadState.prototype.readBuffer = function (length) {
  ...
  const r = this._buffer.slice(this._offset, this._offset + length)
  this._offset += length
  return r
}
