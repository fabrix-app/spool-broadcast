...
export const _Buffer = {
  write: function (B, data, path?) {
    ...
    uint.write(B.length, data, path)
    data.appendBuffer(B)
  },
  read: function (state) {
    const length = uint.read(state)
...
export const string = {
  write: function (s, data, path?) {
    ...
    _Buffer.write(new Buffer(s), data, path)
  },
  read: function (state) {
    return _Buffer.read(state).toString()
  }
}
...
export const json = {
  write: function (j, data, path?) {
    string.write(JSON.stringify(j), data, path)
  },
  read: function (state) {
    return JSON.parse(string.read(state))
  }
}
...
export const regex = {
  write: function (r, data, path?) {
    ...
    string.write(r.source, data, path)
    g = r.global ? 1 : 0
    i = r.ignoreCase ? 2 : 0
    m = r.multiline ? 4 : 0
  ...
  read: function (state) {
    const source = string.read(state),
...
export const date = {
  write: function (d, data, path?) {
    ...
    if (!(d instanceof Date)) {
      throw new TypeError('Expected an instance of Date at ' + path + ', got ' + typeof d + ' ' + d)
    } else if (isNaN(d.getTime())) {
      throw new TypeError('Expected a valid Date at ' + path + ', got ' + d)
    }
    uint.write(d.getTime(), data, path)
  },
  read: function (state) {
    return new Date(uint.read(state))
  }
}
